...
export const Mutation={
    ...
    if(!context.db.users.some((user: { id: any; })=>user.id===cvaddinput.userId))
        throw new Error(`pas de user d'id ${cvaddinput.userId}`)
    else {
     let bool =true;
     ...
     if(!bool)
     throw new Error(` il existe un skills indefini  `)
        ...
        context.pubSub.publish(CV_ADDED, { msg: 'new CV_Added', cv: newcv });
        return newcv
    ...
    updatecv :(parent: any,args: any,context: {
        ...
        context.pubSub.publish(CV_UPDATED, { msg: 'CV_UPDATED', cv: _cv });
    ...
    deleteCv: (parent: any, args: { id: any; }, context: {
       ...
       return false;
        ...
        context.pubSub.publish(CV_DELETED, { msg: 'CV_DELETED', cv: _cv });
